...
import { createWandersReply, listWandersRepliesByCardId, updateWandersReply } from '@/features/wanders/dal/wandersReplies.dal'
  ...
  if (!card.recipient_anon_id && card.sender_anon_id && card.sender_anon_id !== anon.id) {
    const updated = await updateWandersCard({
      ...
      patch: {
        ...
        // Keep channel as link; do not force email/sms here.
    ...
    // Optional: event to capture the moment the receiver "claimed" the thread
    // NOTE: DB allows recipient_claimed / sender_claimed (not "claimed")
...
export async function listRepliesForCard(input) {
  const rows = await listWandersRepliesByCardId({ cardId: input.cardId, limit: input.limit ?? 200 })

...
export default function CardBuilder({
  ...
  // --- 2026 Professional Tailwind inputs (clean, modern, crisp focus) ---
  const labelBase = "block text-[13px] font-semibold tracking-[0.01em] text-gray-800 mb-1.5";

  const inputBase =
    "w-full rounded-2xl border bg-white px-4 py-3 text-[15px] leading-6 " +
    "border-gray-200 text-gray-900 placeholder:text-gray-400 " +
    "shadow-[0_1px_0_0_rgba(0,0,0,0.04)] " +
    "transition duration-150 " +
    "focus:outline-none focus:border-gray-300 focus:ring-4 focus:ring-black/10 " +
    "hover:border-gray-300 " +
    "disabled:bg-gray-50 disabled:text-gray-500 disabled:opacity-70 disabled:cursor-not-allowed";

  const textareaBase = `${inputBase} align-top`;

  const selectBase =
    `${inputBase} ` +
    "appearance-none bg-[linear-gradient(45deg,transparent_50%,rgba(0,0,0,0.55)_50%),linear-gradient(135deg,rgba(0,0,0,0.55)_50%,transparent_50%)] " +
    "bg-[position:calc(100%-18px)_calc(50%+1px),calc(100%-13px)_calc(50%+1px)] bg-[size:5px_5px,5px_5px] bg-no-repeat pr-10";

  const primaryBtn =
    "w-full rounded-2xl bg-black text-white py-3 text-sm font-semibold " +
    "shadow-[0_10px_25px_-15px_rgba(0,0,0,0.45)] " +

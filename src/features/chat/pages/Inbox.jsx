// VERSION: 2025-11-11 (user-inbox + deletethread wiring + deep debug)
...
import { listUserInbox, markConversationRead } from '@/features/chat/api/chatApi.views'
import { useIdentity } from '@/state/identityContext'
import { supabase } from '@/lib/supabaseClient'
import { isAbortError } from '@/features/chat/utils/isAbortError'

// ⬇️ NEW: import the helper you already created (no DB changes)
...
export default function Inbox() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        const userKind = (data || []).find(r => r.actors?.kind === 'user')
        if (!cancelled) setResolvedActorId(userKind?.actor_id ?? null)
      } catch (e) {
        if (!cancelled) setResolvedActorId(null)
        console.error('[Inbox:user] resolveActorIfNeeded failed:', e)
      } finally {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setErrorText('')
      try {
        const raw = await listUserInbox(actorId, { signal: ac.signal })
        const mapped = (raw || []).map(r => ({ id: r.conversation_id, ...r, last_message_preview: '' }))
        if (!cancelled) setRows(mapped)

        // tiny retry to overcome first-RLS cold start
        if (!cancelled && (!rows || rows.length === 0) && mapped.length === 0) {
          setTimeout(async () => {
            if (cancelled) return
            const raw2 = await listUserInbox(actorId, { signal: ac.signal })
            const mapped2 = (raw2 || []).map(r => ({ id: r.conversation_id, ...r }))
            if (!cancelled && mapped2.length) setRows(mapped2)
          }, 250)
        }
      } catch (e) {
        if (isAbortError(e)) return // ignore expected aborts
        console.error('[Inbox:user] load inbox failed:', e)
  ...
  const handleDelete = useCallback(async (conversationId) => {
    ...
    console.groupCollapsed('[Inbox] delete click')
    ...
    try {
      const res = await deleteThreadForMe(
        ...
        { archiveUntilNew: true } // your default behavior
      )

      if (!res?.ok) {
        console.error('[Inbox] delete FAILED', res?.error)
        // rollback UI
        setRows(beforeRows)
      ...
      console.log('[Inbox] delete OK -> returned row:')
      console.table(res.data)
      console.groupEnd()
      // No need to refetch — realtime will patch if this row still existed.
      // We already removed it from the list optimistically.
    } catch (e) {
      if (isAbortError(e)) {
        console.warn('[Inbox] delete aborted by signal')
      } else {
        console.error('[Inbox] delete threw', e)
      }
      // rollback UI
      setRows(beforeRows)
      console.groupEnd()
  ...
  const handlePick = useCallback(async (picked) => {
    ...
    try {
      ...
      try {
        const fresh = await listUserInbox(actorId)
        const mapped = (fresh || []).map(r => ({ id: r.conversation_id, ...r }))
        setRows(mapped)
      } catch {}
      nav(`/chat/${conversationId}?me=${encodeURIComponent(actorId)}`)
    } catch (e) {
      console.error('[Inbox:user] start conversation failed:', e)
      ...
      <div className="mx-auto max-w-3xl">
        <div className="flex items-center justify-between px-3 py-4">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-white/10 hover:bg-white/20 text-white disabled:opacity-50"
          >+ New</button>
        </div>
        ...
        <div className="px-3 py-4 text-xs text-white/40 select-none">Inbox • user • v2025-11-11</div>
      </div>

      ...
      createMediaAssetController({
        ...
        if (postMediaId && mediaAsset?.id) {
          try {
            ...
            captureVcsmError({ feature: 'upload', module: 'recordPostMedia.controller', severity: 'warning', message: `recordPostMediaController: updatePostMediaAssetIdDAL failed — ${e?.message ?? 'unknown'}`, error_name: e?.name, operation: 'updatePostMediaAssetIdDAL', is_handled: true, context: { postId, postMediaId, index: i } })
          }
        } else {
        ...
        captureVcsmError({ feature: 'upload', module: 'recordPostMedia.controller', severity: 'error', message: `recordPostMediaController: createMediaAssetController failed — ${e?.message ?? 'unknown'}`, error_name: e?.name, operation: 'createMediaAssetController', is_handled: true, context: { postId, index: i } })
      })
    )
  )

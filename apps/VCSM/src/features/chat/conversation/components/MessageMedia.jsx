...
export default function MessageMedia({
  ...
  const outerFrameClass = isMine
    ? [
        'mb-1 w-full max-w-full rounded-2xl p-[2px]',
        'bg-gradient-to-br from-purple-400/70 via-purple-500/30 to-fuchsia-400/15',
        className,
      ].join(' ')
    : [
        'mb-1 w-full max-w-full rounded-2xl overflow-hidden',
        className,
      ].join(' ')

  const innerRadius = isMine ? 'rounded-[calc(1rem-1px)]' : 'rounded-2xl'
    ...
    return (
      <div className={outerFrameClass}>
        <button
          type="button"
          onClick={handleOpen}
          className={`relative block w-full text-left active:opacity-95 overflow-hidden ${innerRadius}`}
    ...
    return (
      <div className={outerFrameClass}>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className={`flex items-center gap-3 p-3 bg-black/20 ring-1 ring-white/10 hover:bg-white/5 active:opacity-90 overflow-hidden ${innerRadius}`}

function toneClassByPriority(priority) {
  ...
  if (priority === 'medium') return 'border-purple-300/25 bg-purple-300/10 text-purple-200'
...
export default function BriefingsList({
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-4 text-sm text-white/70">
        Loading briefings...
      </div>
    )
  }
  ...
  if (!items.length) {
    return (
      <div className="rounded-xl border border-dashed border-white/20 bg-white/[0.02] px-3 py-4 text-sm text-white/50">
    ...
    <div className="space-y-2">
      ...
      {items.map((item) => (
        ...
        >
          ...
          {!item.isSeen && (
            <div className="mt-2 inline-flex rounded-full border border-purple-300/25 bg-purple-300/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.08em] text-purple-200">
              unseen
            </div>
          )}
        </button>

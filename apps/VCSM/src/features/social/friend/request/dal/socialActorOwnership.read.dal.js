...
// DAL for the social-graph acting-actor ownership check (social feature).
...
// session user owns user-kind AND vport-kind actors through it). The social graph
// is kind-agnostic: a follower / requester / inbox-owner may be a user OR a vport,
// so the binding must work for either kind (mirrors postActorOwnership.read.dal /
// createPostController; V06A-M1, V06C-M2).
//
 ...
 */
export async function readSocialActorOwnerLinkDAL({ actorId, userId } = {}) {

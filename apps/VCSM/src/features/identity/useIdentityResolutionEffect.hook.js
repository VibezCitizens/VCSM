...
} from "@/features/identity/identitySelfHeal.controller";
import { runFinalizeSelfHeal } from "@/features/identity/identityResolutionSelfHeal.helper";
import { loadIdentity } from "@/features/identity/identityStorage";
import { identityEngineQueryKey } from "@/features/identity/queries/identityEngineQuery";
    ...
    async function run() {
      ...
      try {
        ...
        if (import.meta.env.DEV) {
          debugLoginEvent("IDENTITY_COMMIT_ATTEMPT", {
            phase: "identity",
            status: "info",
            message: `Committing identity for actor ${nextIdentity?.actorId ?? 'unknown'} (${nextIdentity?.kind ?? 'unknown'})`,
            payload: {
        ...
        if (nextIdentity) {
          ...
          if (identityUserId && identityUserId !== user.id) {
            ...
            captureVcsmError({
              feature: 'identity',
              module: 'useIdentityResolutionEffect',
              behavior_id: 'behavior.identity.commit_actor',
              severity: 'fatal',
              message: 'Identity ownership mismatch — resolved identity does not belong to the current session user',
              operation: 'commitIdentity',
              is_handled: false,
              context: {
                ownershipMismatch: true,
                hasIdentity: true,
                hasActorId: !!nextIdentity?.actorId,
                identityKind: nextIdentity?.kind ?? null,
              },
            });
            commitIdentity(null);
            setLoading(false);

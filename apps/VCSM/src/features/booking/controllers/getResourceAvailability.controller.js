...
export async function getResourceAvailabilityController({
  ...
  try {
    ...
    serviceProfiles = serviceIds.length
      ...
      : [];
  } catch (serviceProfilesError) {
    captureVcsmError({
      feature: 'booking',
      module: 'getResourceAvailability.controller',
      behavior_id: 'behavior.booking.availability_read',
      severity: 'warning',
      message: `getResourceAvailabilityController: service profiles fetch failed — availability returned without service duration data — ${serviceProfilesError?.message ?? 'unknown'}`,
      error_name: serviceProfilesError?.name,
      stack: serviceProfilesError?.stack,
      operation: 'listBookingServiceProfilesByServiceIdsDAL',
      is_handled: true,
      context: { resourceFound: true, serviceIdsEmpty: false, dbErrorCode: serviceProfilesError?.code ?? null },
    });
    serviceProfiles = [];
  }

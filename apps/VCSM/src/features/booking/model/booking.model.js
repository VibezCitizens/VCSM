export function mapBookingRow(row) {
  ...
  return {
    ...
    vportActorId: row.profiles?.actor_id ?? null,
    vportName: row.profiles?.name ?? null,
    memberActorId: row.resources?.member_actor_id ?? null,
    memberName: row.resources?.name ?? null,
    ownerActorId: row.profiles?.actor_id ?? row.booking_resources?.owner_actor_id ?? row.resources?.owner_actor_id ?? null,

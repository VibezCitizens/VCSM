 ...
 */
export function useUpdateVportVisibility({ actorId, userId } = {}) {
  const qc = useQueryClient()
  const { refreshVcActorDirectory } = useIdentityOps()

  return useMutation({
    mutationFn: (isPrivate) => ctrlSetActorPrivacy({ actorId, isPrivate, refreshActorFn: refreshVcActorDirectory }),

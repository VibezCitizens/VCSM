  ...
  return (
    <div className="settings-card-surface flex items-center justify-between gap-4 rounded-xl px-4 py-3">
      <div className="flex min-w-0 items-center gap-3">
        <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-white/6">
          {actor?.avatar ? (
            <img src={actor.avatar} alt="" className="h-full w-full object-cover" />
          ) : (
            <div className="flex h-full w-full items-center justify-center text-xs text-white/40">N/A</div>
          )}
        </div>

        <div className="min-w-0">
          <div className="truncate text-sm font-medium text-white/90">{actor?.displayName || 'Unknown'}</div>
          {actor?.username && <div className="truncate text-xs text-white/40">@{actor.username}</div>}
        </div>
      </div>

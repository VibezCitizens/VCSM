...
export default function BottomNavBar() {
  ...
  return (
    ...
    >
      <nav
        className="pointer-events-auto mx-auto flex items-center justify-between gap-1 rounded-full border border-purple-200/15 bg-black/58 px-3 text-white shadow-[0_14px_34px_rgba(2,6,22,0.58)] ring-1 ring-indigo-300/15 backdrop-blur-2xl supports-[backdrop-filter]:bg-black/52"
        style={{
      ...
      >
        <Tab to="/CentralFeed" label={t('nav.home')} icon={<Home size={18} />} end />
...
function ProfileNavTab({ personaActorId, navigate, location, t }) {
  ...
  const { canonicalSlug } = useActorCanonicalSlug(personaActorId)

  const handleClick = useCallback(() => {
    if (import.meta.env.DEV) performance.mark('profile:navigate:start')

    if (!personaActorId) {
      navigate('/CentralFeed')
      return
    }
  ...
  }, [personaActorId, navigate, canonicalSlug])

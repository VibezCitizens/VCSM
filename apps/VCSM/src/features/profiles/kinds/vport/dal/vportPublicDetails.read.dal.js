...
export async function fetchVportPublicDetailsByActorId(actorId) {
  ...
  // is_deleted and is_active are enforced as query filters — not selected or returned.
  // Deleted/inactive VPORTs return null (not found), not a response with lifecycle flags.
  ...
  if (newData) {
    ...
    return {
      ...
      // is_active and is_deleted are NOT returned — enforced at query layer.
      // Any row that reaches here is guaranteed active and non-deleted.

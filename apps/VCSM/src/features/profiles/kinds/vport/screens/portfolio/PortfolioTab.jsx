...
import {
  ...
  SecondaryButton,
  SummaryStat,
} from "@/features/profiles/kinds/vport/screens/portfolio/components/PortfolioTabComponents";
...
export default function PortfolioTab({
  ...
  const servicesState = useVportServices({ targetActorId: actorId, vportType, asOwner: false });
  ...
  const portfolioData = useMemo(
    () => buildVportPortfolioModel({ posts, services, serviceProfiles }),
  ...
  useEffect(() => {
    ...
    if (!stillExists) setActiveFilter(getPortfolioFilterKeyAll());
  ...
  const recentWorkItems = useMemo(() => {
    ...
    if (!candidates.length) candidates = filteredItems.filter((item) => !featuredIds.has(item.id));
    return candidates.length ? candidates : filteredItems;
  }, [filteredItems, featuredTransformations]);

  const relatedServices = useMemo(() => buildRelatedServices(filteredItems), [filteredItems]);
  ...
  if (loadingPosts) return <LoadingState />;
  ...
  return (
    <div className="space-y-5">
      <section className="profiles-portfolio-hero rounded-[1.7rem] p-5 sm:p-6">
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,1fr)] lg:items-end">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
            <SummaryStat icon={ImageIcon} label="Work samples" value={portfolioData.summary.workCount} />
            <SummaryStat icon={Sparkles} label="Transformations" value={portfolioData.summary.transformationCount} />
            <SummaryStat icon={BriefcaseBusiness} label="Categories" value={portfolioData.summary.categoryCount} />
          </div>
        </div>
      </section>

      {featuredTransformations.length ? (
        <PortfolioSection eyebrow="Featured Transformations" title="Before-and-after work" subtitle="Clear paired results make it easy to understand the starting point, the finish, and the level of change this business can deliver.">
          <PortfolioGrid items={featuredTransformations} variant="transformations" renderItem={(item) => (
            <PortfolioTransformationCard key={item.id} item={item} canOpenServices={hasServicesTab} onOpenServices={openServices} />
          )} />
        </PortfolioSection>
      ) : null}

      {recentWorkItems.length ? (
        <PortfolioSection eyebrow="Recent Work" title="Published examples" subtitle="Recent image sets show the kind of projects, finishes, and outcomes this profile is actively producing.">
          <PortfolioGrid items={recentWorkItems} renderItem={(item) => (
            <PortfolioCard key={item.id} item={item} canOpenServices={hasServicesTab} onOpenServices={openServices} />
          )} />
        </PortfolioSection>
      ) : null}

      <PortfolioSection eyebrow="Specialties" title="Tags, categories, and work types" subtitle="These filters help visitors zero in on the kinds of work they care about most, whether that is a transformation, a specific service, or a recurring style.">
        <PortfolioTagChips items={portfolioData.filters} activeKey={activeFilter} onSelect={setActiveFilter} />
      ...
      {relatedServices.length ? (
        <PortfolioSection eyebrow="Service Links" title="Related services" subtitle="When a portfolio entry lines up with a service, the connection appears here so visitors can tie outcomes back to what is offered.">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {relatedServices.map((service) => (
              <RelatedServiceCard key={service.key} service={service} canOpenServices={hasServicesTab} onOpenServices={openServices} />
            ))}
          </div>
        </PortfolioSection>
      ) : null}

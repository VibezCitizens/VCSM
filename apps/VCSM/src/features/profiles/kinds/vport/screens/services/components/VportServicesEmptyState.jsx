...
const CAPABILITY_KEYS = [
  {
    key: "quality",
    icon: ShieldCheck,
    tTitleKey: "vport.servicesView.capQualityTitle",
    subtitle: "Verified service standards and consistency checks.",
  },
  {
    key: "experience",
    icon: Sparkles,
    tTitleKey: "vport.servicesView.capExperienceTitle",
    subtitle: "A curated service stack is being prepared for this vport.",
  },
  {
    key: "launch",
    icon: TimerReset,
    tTitleKey: "vport.servicesView.capLaunchTitle",
...
export default function VportServicesEmptyState({
  ...
  const displayTitle = title ?? t('vport.servicesView.emptyTitle');
  const displaySubtitle = subtitle ?? t('vport.servicesView.emptySubtitle');
  return (
    <section className="relative overflow-hidden rounded-3xl border border-white/12 bg-gradient-to-br from-slate-950/80 via-[#0b1324]/75 to-[#051420]/80 p-5 sm:p-6">
      ...
      <div className="relative">
        <div className="text-base font-semibold text-white sm:text-lg">{displayTitle}</div>
        <div className="mt-1 text-sm text-white/70/75">{displaySubtitle}</div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm font-semibold text-white">{t(item.tTitleKey)}</div>
                </div>

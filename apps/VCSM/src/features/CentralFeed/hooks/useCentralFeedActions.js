...
import { invalidateFeedBlockCache, invalidateFeedFollowCache } from '@/features/CentralFeed/adapters/feedCache.adapter'
...
export function useCentralFeedActions({
  ...
  const handleFollowActor = useCallback(async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.error('[CentralFeed] subscribe failed:', err)
      window.alert(
        err?.message ||
  ...
  }, [actorId, postMenu, closePostMenu, isMenuActorFollowing, toggleFollow, onFollowToast, fetchPosts])
  ...
  const persistHideForMe = useCallback(
    async (postId) => {
      ...
      try {
        ...
        if (import.meta.env.DEV) console.warn('[CentralFeed] persist hide threw:', error)
      }
    },
    [actorId, hidePost]
  )
  ...
  const handleReportSubmit = useCallback(
    async (payload) => {
      ...
      try {
        ...
        if (import.meta.env.DEV) console.error('[CentralFeed] report submit failed:', err)
      }
    },

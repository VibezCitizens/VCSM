...
import { loadDayScheduleController } from "@/features/vportDashboard/dashboard/cards/schedule/controller/loadDaySchedule.controller";
import { createScheduleBooking, updateScheduleBookingStatus, rescheduleScheduleBooking } from "@/features/vportDashboard/dashboard/cards/schedule/controller/scheduleBookingCoordinator.controller";
...
export function useVportOwnerSchedule({ actorId }) {
  ...
  const updateBookingStatus = useCallback(async (bookingId, status) => {
    ...
    setSaveError(null);
    try {
      await updateScheduleBookingStatus({ bookingId, status, callerActorId });
      setDetailModal(null);
      refresh();
    } catch (e) {
  ...
  const rescheduleBooking = useCallback(async ({ bookingId, startsAt, endsAt, resourceId, durationMinutes }) => {
    ...
    setSaveError(null);
    try {
      await rescheduleScheduleBooking({ bookingId, startsAt, endsAt, resourceId, durationMinutes, callerActorId });
      setDetailModal(null);
      refresh();
    } catch (e) {

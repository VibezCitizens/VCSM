...
export function useSubmitBulkFuelPrices({
  ...
  const handleSubmit = useCallback(
    async (rows, values, { shareToFeed = false } = {}) => {
      ...
      // One submission batch id for every fuel submitted in this single action.
      // All citizen submission rows from this submit share it so the station
      // dashboard can group + approve/reject them as one batch. The owner-direct
      // path ignores it (no submission row is created).
      const submissionBatchId =
        typeof crypto !== "undefined" && typeof crypto.randomUUID === "function"
          ? crypto.randomUUID()
          : null;

      // TICKET-FUEL-BATCH-NOTIF-001: when a batch-notification handler is wired,
      // suppress the per-fuel notification and emit ONE grouped notification for
      // the whole batch after all fuels submit. With no handler we fall back to
      // the legacy per-fuel notification so nothing is ever silently dropped.
      const notifyPerFuel = typeof onBatchSubmitted !== "function";
      ...
      try {
        ...
        // One grouped notification for every fuel that actually created a
        // citizen submission row (owner-direct results carry no `submission`,
        // so owner updates emit nothing here).
        if (!notifyPerFuel) {
          const createdFuelKeys = results
            .filter((r) => r.res?.submission)
            .map((r) => r.res.submission.fuelKey ?? r.row.fuelKey)
            .filter(Boolean);
          if (createdFuelKeys.length > 0) {
            await onBatchSubmitted({ submissionBatchId, fuelKeys: createdFuelKeys });
          }
        }
        ...
        if (shareToFeed && onShareToFeed && updatedFuels.length > 0) {
          try {
            publishResult = await onShareToFeed({ updatedFuels });
          } catch (err) {
    ...
    [submitSuggestion, afterSubmitSuggestion, onShareToFeed, onBatchSubmitted]
  );

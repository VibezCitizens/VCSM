...
import { GasStates } from "@/features/vportDashboard/dashboard/cards/gasprices/components/GasStates";
import { OwnerPendingBatchList } from "@/features/vportDashboard/dashboard/cards/gasprices/components/OwnerPendingBatchList";
 ...
 *
 * isOwner: parent screen's verified ownership flag (from useVportDashboardContext
 *   on the dashboard route). Passed through to GasPricesPanel so edit controls are
 *   explicitly owner-gated, not relying on allowOwnerUpdate prop alone.
 *
    ...
    <div style={{ ...CARD_STYLE, marginTop: 14 }}>
      ...
      <div style={{ marginTop: 14 }}>
        <GasStates
          ...
          error={pendingError}
          empty={!pendingLoading && !pendingError && pendingBatches.length === 0}
        />
        {!pendingLoading && !pendingError ? (
          <OwnerPendingBatchList
            batches={pendingBatches}
            officialByFuelKey={officialByFuelKey}
            reviewing={reviewing}
            onApproveAll={onApproveBatch}
            onRejectAll={onRejectBatch}
          />
        ) : null}
      </div>
    </div>

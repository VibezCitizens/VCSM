...
import { useVportPortfolio, usePublishBarbershopPortfolioPost } from "@/features/profiles/kinds/vport/adapters/vportProfiles.adapter";
...
export default function VportDashboardPortfolioScreen() {
  ...
  const { allItems, loading, error, reload, optimisticRemove, optimisticAdd, optimisticUpdate, getItem } =
    useVportPortfolio(targetActorId, { viewerIsOwner: true });
  ...
  const handleCreated = useCallback(
    (optimisticItem, { firstMediaUrl } = {}) => {
      ...
      if (optimisticItem) {
        const withCover = firstMediaUrl ? { ...optimisticItem, coverUrl: firstMediaUrl } : optimisticItem;
        optimisticAdd(withCover);
      }
      if (shareToFeed && isBarbershop) {
        ...
        publishBarbershopPortfolioPost({ portfolioTitle: title, mediaUrl: firstMediaUrl ?? null })
          .catch((err) => {
            if (import.meta.env.DEV) console.error("[feed:publish] barbershop portfolio post failed", err);
          });
      }
    },

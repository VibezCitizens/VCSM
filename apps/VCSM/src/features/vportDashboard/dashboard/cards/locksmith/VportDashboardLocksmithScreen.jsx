...
import {
  ...
  ServiceDetailRow,
  GapServiceRow,
} from "./components/locksmithScreenComponents";
import { useLocksmithDashboard } from "./hooks/useLocksmithDashboard";

export default function VportDashboardLocksmithScreen() {
  const navigate = useNavigate();
  const isDesktop = useDesktopBreakpoint();
  const { vportActorId: targetActorId, canManage: isOwner } = useVportDashboardContext();
  ...
  const {
    ...
    updateArea,
    deleteArea,
  } = useLocksmithDashboard({ actorId: targetActorId });
        ...
        <div style={shell.headerWrap}>
          ...
          <div style={{ padding: 16 }}>
            ...
            <div className="mb-6">
              <div className="mb-3">
                <div className="text-sm font-semibold text-white flex items-center gap-2">
                  <Wrench size={15} /> Service Details
                </div>
                <div className="mt-0.5 text-xs text-white/40">
              ...
              {serviceDetails.length || gapServices.length ? (
                ...
                <div className="rounded-xl border border-white/6 bg-white/[0.02] py-6 text-center text-sm text-white/30">
                  No service details configured yet. Select services first, then configure
                  response time, pricing, and requirements here.
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-white/6 bg-white/[0.02] p-4">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div>
                  <div className="text-lg font-bold text-white">
                    {serviceAreas.filter((a) => a.isEmergencyCovered).length}
                  </div>

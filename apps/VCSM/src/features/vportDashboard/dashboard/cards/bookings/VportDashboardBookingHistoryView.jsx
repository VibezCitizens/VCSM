...
import useVportBookingActions from "@/features/vportDashboard/dashboard/cards/bookings/hooks/useVportBookingActions";
import BookingCard from "@/features/vportDashboard/components/bookingHistory/BookingCard";
import TodayView from "@/features/vportDashboard/components/bookingHistory/TodayView";
import TeamAvailabilityToday from "@/features/vportDashboard/components/bookingHistory/TeamAvailabilityToday";
import QuickBookingModal from "@/features/vportDashboard/components/bookingHistory/QuickBookingModal";
import { filterBookings, groupByDate } from "@/features/vportDashboard/dashboard/cards/bookings/model/vportBookingHistoryView.model";
...
export default function VportDashboardBookingHistoryView({ ownerActorId, viewerActorId }) {
  ...
  // Aggregate bookings across ALL of the vport's resources (e.g. every barber), not just
  // the primary calendar — appointments are booked against the specific staff resource.
  const resourceIds = useMemo(
    () => (resources.items ?? []).map((r) => r.id).filter(Boolean),
    [resources.items]
  );
  ...
  const loading = resources.loading || (resourceIds.length > 0 && histLoading);
      ...
      <div style={shell.container}>
        <div style={shell.headerWrap}>
          ...
          <div style={{ padding: "0 16px 16px" }}>
            ...
            {mode === "today" && (
              <>
                <TeamAvailabilityToday
                  actorId={ownerActorId}
                  refreshKey={availabilityRefresh}
                  onSelectSlot={(slot) => { setBookingPrefill(slot); setShowNewBook(true); }}
                />
                <TodayView
                  bookings={bookings}
                  actions={actions}
                  ...
                  actorId={ownerActorId}
                  onNewBooking={() => { setBookingPrefill(null); setShowNewBook(true); }}
                />
              </>
            )}
  ...
  const activeResourceId = bookingPrefill?.resourceId ?? resourceId;
  const modal = showNewBook && activeResourceId && (
    <QuickBookingModal
      key={`${activeResourceId}:${bookingPrefill?.startTime ?? "new"}`}
      ownerActorId={ownerActorId}
      resourceId={activeResourceId}
      prefill={bookingPrefill}
      onClose={() => { setShowNewBook(false); setBookingPrefill(null); }}
      onCreated={() => { reload(); setAvailabilityRefresh((k) => k + 1); }}
    />
  );

...
import { getVportProfileIdByActorDAL, getVportActorIdByProfileIdDAL } from "@/features/vportDashboard/dal/read/vportProfile.read.dal";
import { readActorVportLinkDAL } from "@/features/vportDashboard/dal/read/actorVport.read.dal";
import { listVportAvailabilityRulesByResourceIdDAL } from "@/features/vportDashboard/dal/read/vportAvailabilityRules.read.dal";
import { listVportBookingsInRangeDAL } from "@/features/vportDashboard/dal/read/vportBookingsInRange.read.dal";
import { getVportServiceByIdDAL } from "@/features/vportDashboard/dal/read/vportServices.read.dal";
...
export async function getVportResourceAvailabilityController({ resourceId, rangeStart, rangeEnd }) {
  try {
    ...
    captureVcsmError({ feature: 'vportDashboard', module: 'bookings.vportPublicBooking.controller', severity: 'error', message: `getVportResourceAvailabilityController: ${error?.message ?? 'unknown'}`, error_name: error?.name, operation: 'getVportResourceAvailability', is_handled: false, context: { dbErrorCode: error?.code ?? null } })
    throw error
  }
}
...
export async function listVportBookingResourcesController({ actorId, includeInactive = false }) {
  try {
    ...
    captureVcsmError({ feature: 'vportDashboard', module: 'bookings.vportPublicBooking.controller', severity: 'error', message: `listVportBookingResourcesController: ${error?.message ?? 'unknown'}`, error_name: error?.name, operation: 'listVportBookingResources', is_handled: false, context: { dbErrorCode: error?.code ?? null } })
    throw error
  }
}
...
export async function createVportPublicBookingController({
  ...
  try {
    ...
    captureVcsmError({ feature: 'vportDashboard', module: 'bookings.vportPublicBooking.controller', severity: 'error', message: `createVportPublicBookingController: ${error?.message ?? 'unknown'}`, error_name: error?.name, operation: 'createVportPublicBooking', is_handled: false, context: { dbErrorCode: error?.code ?? null } })
    throw error
  }
}

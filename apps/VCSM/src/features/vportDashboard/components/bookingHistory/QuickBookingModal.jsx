...
import { useQuickBookingModal } from "@/features/vportDashboard/dashboard/cards/bookings/hooks/useQuickBookingModal";
import ClientSearchField from "@/features/vportDashboard/components/bookingHistory/ClientSearchField";
...
export default function QuickBookingModal({ resourceId, ownerActorId, onClose, onCreated, prefill = null }) {
  const { services, saving, error, setError, createBooking } = useQuickBookingModal({ ownerActorId, resourceId });

  const [form, setForm] = useState({
    date:            prefill?.dateKey ?? prefill?.date ?? todayStr(),
    startTime:       prefill?.startTime ?? nowRoundedTime(),
    durationMinutes: prefill?.durationMinutes ?? 30,
    serviceId:       "",
    customerNote:    "",
  });

  // Client identity: either a selected Citizen (actor-linked) or a free-text walk-in name.
  ...
  async function handleSubmit(e) {
    ...
    // A booking needs either a selected Citizen (actor-linked) or a walk-in name.
    const customerActorId = selectedClient?.actorId ?? null;
    const customerName = selectedClient
      ? (selectedClient.displayName ?? selectedClient.username ?? null)
      : (walkInName.trim() || null);

    if (!customerActorId && !customerName) {
      setError("Select a client or enter a walk-in name.");
      return;
    }
  ...
  return createPortal(
    <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={sheetStyle}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px 14px", borderBottom: "1px solid rgba(148,163,184,.08)" }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: "rgba(255,255,255,.9)" }}>New Booking</div>
            {prefill?.resourceName && (
              <div style={{ fontSize: 12, color: "rgba(167,139,250,.85)", fontWeight: 600, marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {prefill.resourceName}
              </div>
            )}
          </div>
          <button type="button" onClick={onClose} style={{ width: 30, height: 30, borderRadius: 8, border: "none", background: "rgba(255,255,255,.07)", color: "rgba(255,255,255,.5)", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>×</button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: "auto", padding: "16px 18px 0", display: "grid", gap: 14 }}>
          ...
          {/* Client — searchable Citizen selector with walk-in fallback */}
          <ClientSearchField
        ...
        {/* Footer */}
        <div style={{ padding: "14px 18px", borderTop: "1px solid rgba(148,163,184,.07)" }}>
          <button
            type="submit"
            form=""

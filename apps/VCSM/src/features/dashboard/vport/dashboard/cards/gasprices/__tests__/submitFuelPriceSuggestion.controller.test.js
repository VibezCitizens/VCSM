...
import { submitFuelPriceSuggestionController } from "@/features/dashboard/vport/dashboard/cards/gasprices/controller/submitFuelPriceSuggestion.controller.js";
...
vi.mock(
  "@/features/profiles/kinds/vport/dal/services/resolveVportProfileId.dal",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/dal/vportFuelPrices.read.dal",
  () => ({
    fetchVportFuelPricesDAL: vi.fn(),
    invalidateFuelPriceCache: vi.fn(),
  })
);

vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/dal/vportStationPriceSettings.read.dal",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/dal/vportFuelPriceSubmissions.write.dal",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/dal/vportFuelPrices.write.dal",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/dal/vportFuelPriceHistory.write.dal",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/model/vportFuelPrice.model",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/model/vportFuelPriceSubmission.model",
  () => ({
...
vi.mock(
  "@/features/dashboard/vport/dashboard/cards/gasprices/model/vportStationPriceSettings.model",
  () => ({
...
} from "@/features/dashboard/vport/dashboard/cards/gasprices/dal/vportFuelPrices.read.dal";
...
describe("submitFuelPriceSuggestionController — owner path (ownerUpdate: true)", () => {
  ...
  it("does NOT call invalidateFuelPriceCache when ownership check fails", async () => {
    checkVportOwnershipController.mockResolvedValue(false);
    ...
    expect(invalidateFuelPriceCache).not.toHaveBeenCalled();
  });
  ...
  it("calls invalidateFuelPriceCache(targetActorId) after successful owner write", async () => {
    checkVportOwnershipController.mockResolvedValue(true);
    ...
    expect(invalidateFuelPriceCache).toHaveBeenCalledOnce();
    expect(invalidateFuelPriceCache).toHaveBeenCalledWith(TARGET_ACTOR_ID);
  });
...
describe("submitFuelPriceSuggestionController — citizen path (ownerUpdate: false)", () => {
  ...
  it("does NOT call invalidateFuelPriceCache on the citizen path", async () => {
    await submitFuelPriceSuggestionController({
      ...baseArgs,
      ownerUpdate: false,
    });

...
export async function onRequest(context) {
  ...
  if (env.SUPABASE_URL && env.SUPABASE_ANON_KEY) {
    try {
      rows = await fetchAllPages(env, 'profiles', 'slug=not.is.null&is_active=eq.true&is_deleted=eq.false&directory_visible=eq.true&directory_status=eq.listed&select=slug,updated_at,profile_public_details!inner(profile_id)');
    } catch {
      rows = [];
    }
  }

...
export async function onRequest(context) {
  ...
  const canonical = `${origin}/p/${slug}/book`;
  const fallbackImage = `${origin}/VportBusinnesCard.jpeg`;

  let title = "Book on WanderEx | Vibez Citizens";
  let description = "Send a booking request lead on WanderEx.";
  let image = fallbackImage;

  const profile = await readPublicProfileBySlug({ slug, env, origin });
  if (profile) {
    title = `Book ${profile.name} | WanderEx`;
    description = `Send a booking request to ${profile.name} on WanderEx. No login required.`;

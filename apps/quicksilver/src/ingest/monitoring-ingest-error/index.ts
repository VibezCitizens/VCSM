...
serve(async (req: Request) => {
  ...
  if (!project_id || typeof project_id !== "string" || !project_id.trim()) {
    return json({ ok: false, error: "project_id is required", code: "INVALID_INPUT" }, 400, cors)
  }
  ...
  if (!message || typeof message !== "string" || !message.trim()) {
    return json({ ok: false, error: "message is required", code: "INVALID_INPUT" }, 400, cors)
  }

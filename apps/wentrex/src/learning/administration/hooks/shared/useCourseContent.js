...
import { getCourseContentController } from "@/learning/administration/controller/shared/getCourseContent.controller";
  ...
  const reload = useCallback(async () => {
    ...
    if (!supabase || !actorId || !realmId || !courseId) {
      const nextError = {
        ...
        message: "useCourseContent requires supabase, actorId, realmId, and courseId",
      };
    ...
    try {
      const result = await getCourseContentController({
        supabase,
        actorId,
        realmId,
        courseId,

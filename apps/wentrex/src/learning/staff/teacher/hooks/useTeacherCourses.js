  ...
  const reload = useCallback(async () => {
    ...
    if (!supabase || !actorId || !realmId) {
      const nextError = {
        ...
        message: "useTeacherCourses requires supabase, actorId, and realmId",
      };
    ...
    try {
      const result = await listTeacherCoursesController({
        supabase,
        actorId,
        realmId,
      });
      ...
      const nextError = {
        code: "DB_ERROR",
        message: err?.message ?? "Failed to load teacher courses",
      };

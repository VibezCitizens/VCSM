-- Organization member onboarding RPC
-- Purpose:
-- - keep organization memberships actor-based
-- - resolve auth.users -> public.profiles -> vc.actors -> vc.actor_owners
-- - create or reuse the relational records needed to enroll a member in one school
-- - keep public profiles private by default
...
is 'Creates or reuses the profile, actor, actor_owner, and organization_membership rows needed to enroll one organization member.';

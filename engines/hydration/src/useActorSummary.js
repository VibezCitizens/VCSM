...
// ============================================================
// Consumer Hook — lightweight store selector for actor display
// ============================================================
// Remains cheap/read-only. Does NOT trigger network requests.
// Exposes `missing` flag for debuggability.
// ============================================================
...
/**
 * useActorSummary — presentation hook for actor display data.
 *
 * @param {string|Object} actorRef — actor ID string or object with actor fields
 * @returns {{ actorId, displayName, username, avatar, route, missing, stale }}
 */
export function useActorSummary(actorRef) {
  ...
  return useMemo(() => {
    const missing = actorId != null && actor == null
    const stale = actor != null && actor._hydratedAt != null
      ? Date.now() - actor._hydratedAt > 5 * 60 * 1000
      : false

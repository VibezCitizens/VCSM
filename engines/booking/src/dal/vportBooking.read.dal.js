...
const BOOKING_SELECT_PUBLIC = [
  'id', 'resource_id', 'starts_at', 'ends_at', 'status', 'duration_minutes',
  'created_by_actor_id',
].join(',')
...
export async function dalGetVportBookingById({ bookingId }) {
...
export async function dalListVportBookingsInRange({ resourceId, rangeStart, rangeEnd, statuses = null, publicMode = false }) {
...
export async function dalListVportBookingsByResource({ resourceId, statuses = null, limit = 50, offset = 0 }) {
  if (!resourceId) throw new Error('BookingEngine: resourceId is required')
